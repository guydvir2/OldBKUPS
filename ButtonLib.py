...
class UpDownButton(tk.Frame):
    def __init__(self, master, nickname='', text_up='UP', text_down='DOWN', width=15, height=3, hw_in=[], hw_out=[],
        ...
        self.status_label_var.set('Status: None')
        self.but_stat = []
    ...
    def run_schedule(self):
        ...
        def check_state(sch_stat, task_stat, sw):
            # sw =0,1 for up,down
            sw_txt = [self.text_up, self.text_down]

            # Switch On/Off -------** **-----in task no.     relative task_state is On -------_**
            if not sch_stat[0][0] == -1 and task_stat[sch_stat[0][1]] == 1:
                if not bool(sch_stat[0][0]) == self.get_state()[0]:
                    self.ext_press(sw, sch_stat[0][0], "Schedule Switch")
            ...
            if sch_stat[0][0] == 1 and task_stat[sch_stat[0][1]] == 1:
                self.status_label_var.set("'%s' OFF :\n"%sw_txt[sw] + str(sch_stat[2]))
            # if in "off state": time to next on, in all tasks
            elif sch_stat[0][0] == -1:
                self.status_label_var.set("'%s' ON :\n"%sw_txt[sw] + str(sch_stat[2]))
            elif sch_stat[0][0] == 1 and task_stat[sch_stat[0][1]] == 0:
                self.status_label_var.set("Task %s, canclled" % str(sch_stat[0][1]))

        if self.up:  # there is an UP schedule
            # Run Schedule
            self.SchRun_Up.run_schedule()
            sched_status_up = self.SchRun_Up.get_status()
            check_state(sched_status_up, self.task_state_up, 0)

        if self.down:
            # Run Schedule
            self.SchRun_Down.run_schedule()
            sched_status_down = self.SchRun_Down.get_status()
            check_state(sched_status_down, self.task_state_down, 1)
    ...
    def build_gui(self, text_up, text_down, width, height):
        ...
        self.status_label = tk.Label(self, textvariable=self.status_label_var)
    ...
    def switch_logic(self, sw):
        ...
        # sw = 0 for up button
        # sw = 1 for down button

        sw_i = [0, 1]
        if self.but_stat[sw_i[sw]].get() == 1:
            if self.but_stat[sw_i[sw - 1]].get() == 1:
                self.but_stat[sw_i[sw - 1]].set(0)
                self.execute_command(sw_i[sw], self.but_stat[sw_i[sw - 1]].get())
                sleep(1)
                self.execute_command(self.but_stat[sw_i[sw]].get(), self.but_stat[sw_i[sw - 1]].get())

            elif self.but_stat[sw_i[sw - 1]].get() == 0:
                self.execute_command(self.but_stat[sw_i[sw]].get(), self.but_stat[sw_i[sw - 1]].get())

        elif self.but_stat[sw_i[sw]].get() == 0:
            self.execute_command(self.but_stat[sw_i[sw]].get(), self.but_stat[sw_i[sw - 1]].get())
    ...
    def execute_command(self, up_stat, down_stat):
        ...
        print("self.HW_output.set_state-", 0, up_stat)
        print("self.HW_output.set_state-", 1, down_stat)
...
class ToggleButton(tk.Frame):
    ...
    def build_gui(self, master, nickname, height, width):
        ...
        button = tk.Checkbutton(self, text=nickname, variable=self.but_vat, indicatoron=0, height=height, width=width,
...
root = tk.Tk()
a = UpDownButton(root, hw_out=[4, 27], ip_out='192.168.2.113', nickname="UpDown", hw_in=[21, 22],
                 sched_vector_down=[[[2], "11:37:30", "11:38:00"], [[2], "18:26:40", "19:59:10"]],
                 sched_vector=[[[1], "11:37:30", "11:38:00"], [[2], "14:26:40", "19:59:10"]])
a.grid()
root.mainloop()
